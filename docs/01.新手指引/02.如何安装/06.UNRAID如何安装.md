...
[下载并解压](http://310628.xyz:36668/MSGO_UNRAID.zip)，得到三个XML模板文件 
...
<Container version="2">
  ...
  <Repository>qqaazz.msaber.fun/xylplm/media-saber:go-dev</Repository>
  ...
  <Config Name="MS_PORT" Target="MS_PORT" Default="" Mode="" Description="" Type="Variable" Display="always" Required="false" Mask="false">4000</Config>
...
依次进入 依次进入 `boot` → `config` → `plugins` → `dockerMan` → `templates-user` 文件夹
